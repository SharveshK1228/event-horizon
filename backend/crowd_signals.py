...
class QualityReference:
    ...
    def update(self, timestamp, features):
        self.tile_states = ['WARMING UP']*len(features)
        ...
        if self.reference is None:
            ...
            if timestamp-self.start >= 3 and len(self.samples) >= 10:
                ...
                self.samples.clear()
            return 'WARMING UP',0
        bad = [c < max(rc*self.ratio,3) or sharp < max(rs*self.ratio,2) or exposure > max(re+.35,.8)
               for (c,sharp,exposure),(rc,rs,re) in zip(features,self.reference)]
        self.tile_states = ['DEGRADED' if value else 'REFERENCE-LIKE' for value in bad]
        changed = sum(bad)
